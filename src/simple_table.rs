...
pub struct SimpleTable<T: 'static + SimpleModel> {
    pub table: Table,
    pub model: &'static mut T,
}
...
impl<T: 'static + SimpleModel> SimpleTable<T> {
    pub fn new(model: &'static mut T) -> SimpleTable<T> {
        // initialize table
    ...
    pub fn redraw(&mut self) {
        let row_count = self.model.row_count() as i32;
